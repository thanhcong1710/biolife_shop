...
abstract class PHPRtfLite_Container_Base{
     ...
     * @return  PHPRtfLite_Element
     */
    public function clearhtmlnotspecial($text){
        $text = preg_replace("/<a(.*?)>/msi", "", $text);
        $text = preg_replace("/<\/a[ ]*>/msi", "", $text);

        $text = preg_replace("/<p(.*?)>/msi", "", $text);
        $text = preg_replace("/<\/p[ ]*>/msi", "", $text);

        $text = preg_replace("/<span(.*?)>/msi", "", $text);
        $text = preg_replace("/<\/span[ ]*>/msi", "", $text);

        $text = preg_replace("/<div(.*?)>/msi", "", $text);
        $text = preg_replace("/<\/div[ ]*>/msi", "", $text);
    ...
    public function writeText($text,
                              ...
                              $convertTagsToRtf = true, $pageSetup = array('endPage'=>false,'orentation'=>false))
    {
        // $endPage = false, $setLandScape = false
        $partern = array("\n",' ',"< ?\/? ?br ?\/? ?>");
        $text = $this->clearhtmlnotspecial($text);
        // $text = preg_replace("/<a(.*?)>/msi", "", $text);
        // $text = preg_replace("/<\/a[ ]*>/msi", "", $text);
        // $text = preg_replace("/<p(.*?)>/msi", "", $text);
        // $text = preg_replace("/<\/p[ ]*>/msi", "", $text);
        // $text = preg_replace("/<span(.*?)>/msi", "", $text);
        // $text = preg_replace("/<\/span[ ]*>/msi", "", $text);
        $text = $this->ltrim($this->rtrim($text,$partern),$partern);
        
        if (preg_match("/(<i>.+)|(.+<\/i>)/msi", $text)) {
            // print_r($text);<br/>
            $subject = "</i>\t\n          <i>";
            $text_italic = preg_replace("/\n/", $subject, $text);
            // print_r($text_italic);
            $element = new PHPRtfLite_Element($this->_rtf, $text_italic, $font, $parFormat);
            if ($convertTagsToRtf) {
            ...
            if(!preg_match("/<table(.*?)<\/table>/msi", $text)) {
                ...
                foreach ($str as $key => $text) {
                    ...
                    if($key == count($str)-1 && $pageSetup['endPage']){
                        $element->setPageBreak();
                    }
                    if($key == 0){
                        $element->setOrentation($pageSetup['orentation']);
                    }
                    $element->setConvertTagsToRtf();
                    $this->_elements[] = $element;
                }
                ...
                foreach ($strings as $key => $value) {
                    if($key % 2 == 1){
                        ...
                        $str = $this->ltrim($this->rtrim($value,$partern),$partern);
                        $str = explode("\n",$str);
                        foreach ($str as $key => $text) {
                            $element = new PHPRtfLite_Element($this->_rtf, $text, $font, $parFormat);
                            $element->setConvertTagsToRtf();
                            if($pageSetup['endPage'] && $key == count($strings)-1){
                                $element->setPageBreak();
                            }
                            $this->_elements[] = $element;
                        }
    ...
    function ltrim($text,$dilimiter){
        if(!is_array($dilimiter)){
            if(preg_match("/^$dilimiter(.*?)/",$text)){
                $text = preg_replace("/^$value/","",$text);
                $text = $this->ltrim($text,$dilimiter);
            }
        }else{
    ...
    function rtrim($text,$dilimiter){
        if(!is_array($dilimiter)){
            if(preg_match("/$dilimiter$/",$text)){
                $text = $this->rtrim(rtrim(substr($text,0,strlen($text) - strlen($dilimiter))),$dilimiter);
            }
        }else{
            foreach ($dilimiter as $key => $value) {
                if(preg_match("/$value$/",$text)){
                    $text = preg_replace("/$value$/","",$text);
                    $text = $this->rtrim($text,$dilimiter);
                }
            }
        }
        return $text;
